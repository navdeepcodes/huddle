...
describe("SYSTEM_PROMPT - asset/photography discipline (Phase 25 regression)", () => {
  it("is honest that images are hotlinked, not self-hosted, and treats that as a real reliability risk", () => {
    expect(SYSTEM_PROMPT.toLowerCase()).toContain("cannot generate, fetch, or self-host binary image files");
  });
